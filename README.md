<h1 align="center">Hi! Welcome to my GitHub!</h1>
<p align="center">
  <em>Software Engineer / Freelance / Tokyo</em>
</p>

<p align="center">
  <a href="https://mohablog.com"><img src="https://img.shields.io/badge/Blog-mohablog-333?style=flat-square" alt="Blog" /></a>
  <a href="https://qiita.com/moha0918_"><img src="https://img.shields.io/badge/Qiita-moha0918__-55C500?style=flat-square&logo=qiita&logoColor=white" alt="Qiita" /></a>
  <a href="https://github.com/mohadayo"><img src="https://img.shields.io/badge/GitHub-mohadayo-181717?style=flat-square&logo=github" alt="GitHub" /></a>
</p>

---

### Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,go,ts,vue,fastapi,django,docker,aws,postgres,githubactions&perline=5&theme=light" alt="Tech Stack" />
</p>

---

### Works

- [rootage-quiz](https://github.com/mohadayo/rootage-quiz) - SES企業向け新人エンジニア育成アプリ (Go + Vue 3 + PostgreSQL)
  <br>*[ルーテイジ株式会社](https://www.rootage.co.jp/)様より受注・納品。許可を得てソースコードを公開しています。*
- [rag-support-assist](https://github.com/mohadayo/rag-support-assist) - カスタマーサポート回答支援AI (FastAPI + Next.js + pgvector)
  <br>*FAQ・規約・マニュアルをRAG検索し、回答候補を自動生成。[Demo](https://rag-support-assist.vercel.app)*
- [url-shortener](https://github.com/mohadayo/url-shortener) - URL短縮サービス (Go + Python + TypeScript)
  <br>*マルチ言語構成のAPI設計デモ。SSRF対策・レート制限実装済み。[Demo](https://moha-url-shortener.fly.dev)*

---

### Blog Posts
<!-- blog -->
- [PythonでAIスキル開発：UI/UXデザイン知識をAIに実装する方法](https://mohablog.com/python-ai-ui-ux-skill-development/) (2026-04-08)
- [Claude CodeでAPI設計ドキュメントから型安全なクライアント実装を自動生成する](https://mohablog.com/openapi-typescript-python-client-generation/) (2026-04-08)
- [Deer-Flowで長期的なAgentタスクを実装する方法](https://mohablog.com/deer-flow-agentic-framework-python/) (2026-04-07)
<!-- /blog -->

### Qiita Posts
<!-- qiita -->
- [複数のAIが同時に働く仕組み - Agent Teamsで実現するチーム開発の全体像](https://qiita.com/moha0918_/items/9682b1ec8b71e884ec13) (2026-04-09)
- [今日のClaude Code v2.1.97 リリース｜毎日Changelog解説](https://qiita.com/moha0918_/items/112149866223f8b5f6a0) (2026-04-09)
- [Claude Code Headless実行で開発効率40%改善 - 導入ガイド完全版](https://qiita.com/moha0918_/items/998f0ca62960492a13d4) (2026-04-08)
<!-- /qiita -->

---

<p align="center">
  <sub>Last updated: 2026/04/09 10:38 JST</sub>
</p>
